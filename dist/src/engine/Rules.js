...
function currentPlayerId(players, index) {
...
function findCardInHand(hand, cardUid) {
...
export function getValidMoves(state, playerId) {
    ...
    if (state.phase === "BIDDING") {
        if (!state.bidding)
            throw new Error("Missing bidding state");
    ...
    if (state.phase === "PLAY") {
        if (!state.play)
            throw new Error("Missing play state");
        ...
        return validCards.map((c) => ({ type: "PlayCard", playerId, cardUid: c.uid }));
    }
...
export function applyMove(state, move) {
    ...
    const isValid = valid.some((m) => {
        if (m.type !== move.type)
            return false;
        if (m.playerId !== move.playerId)
            return false;
        if (m.type === "Bid")
            return move.amount === m.amount && move.trump === m.trump;
        if (m.type === "PlayCard")
            return move.cardUid === m.cardUid;
    ...
    if (state.phase === "BIDDING") {
        if (!state.bidding)
            throw new Error("Missing bidding state");
        const nextBidding = applyBiddingMove(state.players, state.bidding, move);
        if (!isBiddingComplete(nextBidding)) {
            return { ...state, bidding: nextBidding };
        }
        const highest = nextBidding.highestBid;
        if (!highest)
            throw new Error("Bidding completed without a highest bid");
