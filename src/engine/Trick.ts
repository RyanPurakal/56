...
export function cardsOfSuit(hand: ReadonlyArray<Card>, suit: Suit): ReadonlyArray<Card> {
  return hand.filter((c) => c.suit === suit);
}

export function validPlaysForFollowSuit(hand: ReadonlyArray<Card>, leadSuit: Suit | null): ReadonlyArray<Card> {
...
export function resolveTrickWinner(
  trickPlays: ReadonlyArray<TrickPlay>,
  trump: Suit
): { winnerId: PlayerId; winningCard: Card } {
  if (trickPlays.length !== 4) throw new Error("Trick must have 4 plays to resolve");

  const leadSuit = trickPlays[0]!.card.suit;

  const compare = (a: TrickPlay, b: TrickPlay): number => {
  ...
  for (let i = 1; i < trickPlays.length; i++) {
    ...
    if (compare(cur, best) > 0) best = cur;
  }

  return { winnerId: best.playerId, winningCard: best.card };
}

export function nextTurnIndex(players: ReadonlyArray<Player>, currentIndex: number): number {
  return (currentIndex + 1) % players.length;
}

export function startNewTrick(leaderId: PlayerId): Trick {
  return { leaderId, plays: [] };
}

export function appendPlay(trick: Trick, play: TrickPlay): Trick {
